...
public class LeastAtLastPropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public LeastAtLastPropertiesEditionComponent(EObject leastAtLast, String editing_mode) {
		if (leastAtLast instanceof LeastAtLast) {
			this.leastAtLast = (LeastAtLast)leastAtLast;
			if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
				semanticAdapter = initializeSemanticAdapter();
				this.leastAtLast.eAdapters().add(semanticAdapter);
			}
		}
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(final Notification msg) {
				if (basePart == null)
					LeastAtLastPropertiesEditionComponent.this.dispose();
				else {
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return TarotViewsRepository.LeastAtLast.class;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (leastAtLast != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (LeastAtLastPropertiesEditionPart)provider.getPropertiesEditionPart(TarotViewsRepository.LeastAtLast.class, kind, this);
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if (key == TarotViewsRepository.LeastAtLast.class)
			this.basePart = (LeastAtLastPropertiesEditionPart) propertiesEditionPart;
	}
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (basePart != null && key == TarotViewsRepository.LeastAtLast.class) {
			((IPropertiesEditionPart)basePart).setContext(elt, allResource);
			final LeastAtLast leastAtLast = (LeastAtLast)elt;
			// init values
			basePart.initSide((EEnum) TarotPackage.eINSTANCE.getBonus_Side().getEType(), leastAtLast.getSide());
			// init filters
	...
	public Diagnostic validate() {
		...
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			EObject copy = EcoreUtil.copy(leastAtLast);
			copy = getPropertiesEditionObject(copy);
			validate =  EEFRuntimePlugin.getEEFValidator().validate(copy);
		}
		else if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode))
			validate = EEFRuntimePlugin.getEEFValidator().validate(leastAtLast);
	...
	public void dispose() {
		if (semanticAdapter != null)
			leastAtLast.eAdapters().remove(semanticAdapter);
	}
