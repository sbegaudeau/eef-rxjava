...
public class MiserePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public MiserePropertiesEditionComponent(EObject misere, String editing_mode) {
		if (misere instanceof Misere) {
			this.misere = (Misere)misere;
			if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
				semanticAdapter = initializeSemanticAdapter();
				this.misere.eAdapters().add(semanticAdapter);
			}
		}
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(final Notification msg) {
				if (basePart == null)
					MiserePropertiesEditionComponent.this.dispose();
				else {
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return TarotViewsRepository.Misere.class;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (misere != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (MiserePropertiesEditionPart)provider.getPropertiesEditionPart(TarotViewsRepository.Misere.class, kind, this);
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if (key == TarotViewsRepository.Misere.class)
			this.basePart = (MiserePropertiesEditionPart) propertiesEditionPart;
	}
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (basePart != null && key == TarotViewsRepository.Misere.class) {
			((IPropertiesEditionPart)basePart).setContext(elt, allResource);
			final Misere misere = (Misere)elt;
			// init values
			// init part
			basePart.initDeclarer(allResource, misere.getDeclarer());
			// set the button mode
			basePart.setDeclarerButtonMode(ButtonsModeEnum.BROWSE);
			// init filters
	...
	public Diagnostic validate() {
		...
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			EObject copy = EcoreUtil.copy(misere);
			copy = getPropertiesEditionObject(copy);
			validate =  EEFRuntimePlugin.getEEFValidator().validate(copy);
		}
		else if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode))
			validate = EEFRuntimePlugin.getEEFValidator().validate(misere);
	...
	public void dispose() {
		if (semanticAdapter != null)
			misere.eAdapters().remove(semanticAdapter);
	}
