...
public class HandfulPropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public HandfulPropertiesEditionComponent(EObject handful, String editing_mode) {
		if (handful instanceof Handful) {
			this.handful = (Handful)handful;
			if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
				semanticAdapter = initializeSemanticAdapter();
				this.handful.eAdapters().add(semanticAdapter);
			}
		}
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(final Notification msg) {
				if (basePart == null)
					HandfulPropertiesEditionComponent.this.dispose();
				else {
	...
	protected void runUpdateRunnable(final Notification msg) {
		...
		if (TarotPackage.eINSTANCE.getHandful_Size().equals(msg.getFeature()) && basePart != null)
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return TarotViewsRepository.Handful.class;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (handful != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (HandfulPropertiesEditionPart)provider.getPropertiesEditionPart(TarotViewsRepository.Handful.class, kind, this);
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if (key == TarotViewsRepository.Handful.class)
			this.basePart = (HandfulPropertiesEditionPart) propertiesEditionPart;
	}
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (basePart != null && key == TarotViewsRepository.Handful.class) {
			((IPropertiesEditionPart)basePart).setContext(elt, allResource);
			final Handful handful = (Handful)elt;
			// init values
			basePart.initSide((EEnum) TarotPackage.eINSTANCE.getBonus_Side().getEType(), handful.getSide());
			basePart.initSize((EEnum) TarotPackage.eINSTANCE.getHandful_Size().getEType(), handful.getSize());
			// init filters
	...
	public EObject getPropertiesEditionObject(EObject source) {
		if (source instanceof Handful) {
			Handful handfulToUpdate = (Handful)source;
	...
	public void firePropertiesChanged(IPropertiesEditionEvent event) {
		if (!isInitializing()) {
			...
			if (TarotViewsRepository.Handful.side == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, handful, TarotPackage.eINSTANCE.getBonus_Side(), event.getNewValue()));
	...
	public Diagnostic validate() {
		...
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			EObject copy = EcoreUtil.copy(handful);
			copy = getPropertiesEditionObject(copy);
			validate =  EEFRuntimePlugin.getEEFValidator().validate(copy);
		}
		else if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode))
			validate = EEFRuntimePlugin.getEEFValidator().validate(handful);
	...
	public void dispose() {
		if (semanticAdapter != null)
			handful.eAdapters().remove(semanticAdapter);
	}
