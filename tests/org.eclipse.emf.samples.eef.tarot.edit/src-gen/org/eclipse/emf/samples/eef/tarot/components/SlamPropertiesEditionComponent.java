...
public class SlamPropertiesEditionComponent extends StandardPropertiesEditionComponent {
	...
	public SlamPropertiesEditionComponent(EObject slam, String editing_mode) {
		if (slam instanceof Slam) {
			...
			if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
				...
				this.slam.eAdapters().add(semanticAdapter);
			}
		}
	...
	private AdapterImpl initializeSemanticAdapter() {
		return new EContentAdapter() {
			...
			public void notifyChanged(final Notification msg) {
				if (basePart == null)
					SlamPropertiesEditionComponent.this.dispose();
				else {
	...
	protected void runUpdateRunnable(final Notification msg) {
		...
		if (TarotPackage.eINSTANCE.getSlam_Announced().equals(msg.getFeature()) && basePart != null)
			basePart.setAnnounced((Boolean)msg.getNewValue());

		if (TarotPackage.eINSTANCE.getSlam_Achieved().equals(msg.getFeature()) && basePart != null)
	...
	public java.lang.Class translatePart(String key) {
		if (BASE_PART.equals(key))
			return TarotViewsRepository.Slam.class;
	...
	public IPropertiesEditionPart getPropertiesEditionPart(int kind, String key) {
		if (slam != null && BASE_PART.equals(key)) {
			if (basePart == null) {
				...
				if (provider != null) {
					basePart = (SlamPropertiesEditionPart)provider.getPropertiesEditionPart(TarotViewsRepository.Slam.class, kind, this);
	...
	public void setPropertiesEditionPart(java.lang.Class key, int kind, IPropertiesEditionPart propertiesEditionPart) {
		if (key == TarotViewsRepository.Slam.class)
			this.basePart = (SlamPropertiesEditionPart) propertiesEditionPart;
	}
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (basePart != null && key == TarotViewsRepository.Slam.class) {
			((IPropertiesEditionPart)basePart).setContext(elt, allResource);
			final Slam slam = (Slam)elt;
			// init values
			basePart.initSide((EEnum) TarotPackage.eINSTANCE.getBonus_Side().getEType(), slam.getSide());
			basePart.setAnnounced(slam.isAnnounced());

			basePart.setAchieved(slam.isAchieved());
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if ((slam != null) && (basePart != null)) { 
			cc.append(SetCommand.create(editingDomain, slam, TarotPackage.eINSTANCE.getBonus_Side(), basePart.getSide()));
	...
	public EObject getPropertiesEditionObject(EObject source) {
		if (source instanceof Slam) {
			Slam slamToUpdate = (Slam)source;
			slamToUpdate.setSide((PLAYER_SIDE)basePart.getSide());
	...
	public void firePropertiesChanged(IPropertiesEditionEvent event) {
		if (!isInitializing()) {
			...
			if (TarotViewsRepository.Slam.side == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, slam, TarotPackage.eINSTANCE.getBonus_Side(), event.getNewValue()));

			if (TarotViewsRepository.Slam.announced == event.getAffectedEditor())
				command.append(SetCommand.create(liveEditingDomain, slam, TarotPackage.eINSTANCE.getSlam_Announced(), event.getNewValue()));
	...
	public Diagnostic validateValue(IPropertiesEditionEvent event) {
		...
		if (event.getNewValue() != null) {
			...
			try {
				...
				if (TarotViewsRepository.Slam.announced == event.getAffectedEditor()) {
					Object newValue = EcoreUtil.createFromString(TarotPackage.eINSTANCE.getSlam_Announced().getEAttributeType(), newStringValue);
					ret = Diagnostician.INSTANCE.validate(TarotPackage.eINSTANCE.getSlam_Announced().getEAttributeType(), newValue);
				}
				if (TarotViewsRepository.Slam.achieved == event.getAffectedEditor()) {
	...
	public Diagnostic validate() {
		...
		if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {
			EObject copy = EcoreUtil.copy(slam);
			copy = getPropertiesEditionObject(copy);
			validate =  EEFRuntimePlugin.getEEFValidator().validate(copy);
		}
		else if (IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode))
			validate = EEFRuntimePlugin.getEEFValidator().validate(slam);
	...
	public void dispose() {
		if (semanticAdapter != null)
			slam.eAdapters().remove(semanticAdapter);
	}
