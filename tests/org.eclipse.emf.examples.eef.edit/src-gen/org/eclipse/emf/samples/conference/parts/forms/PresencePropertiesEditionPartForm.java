...
public class PresencePropertiesEditionPartForm extends SectionPropertiesEditingPart implements IFormPropertiesEditionPart, PresencePropertiesEditionPart {
	 ...
	 */
	public PresencePropertiesEditionPartForm() { super(); }
	...
	public void initAssists(ReferencesTableSettings settings) {
		...
		boolean readOnly = isReadOnly(ConferenceViewsRepository.Presence.Talks.assists);
		if (readOnly && assists.getTable().isEnabled()) {
			assists.setEnabled(false);
			assists.setToolTipText(ConferenceMessages.Presence_ReadOnly);
		} else if (!readOnly && !assists.getTable().isEnabled()) {
