...
public class TextPropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * The Base part
	 */
	private TextPropertiesEditionPart basePart;
			...
			public void notifyChanged(Notification msg) {
				if (basePart == null)
					...
					if (NonregPackage.eINSTANCE.getAdress_PostalCode().equals(msg.getFeature()) && basePart != null)
						basePart.setText(((Integer)msg.getNewValue()).toString());

					if (NonregPackage.eINSTANCE.getAdress_PostalCode().equals(msg.getFeature()) && basePart != null)
						basePart.setTextRO(((Integer)msg.getNewValue()).toString());
	...
	public void initPart(java.lang.Class key, int kind, EObject elt, ResourceSet allResource) {
		if (basePart != null && key == NonregViewsRepository.Text.class) {
			...
			basePart.setTextRO(String.valueOf(adress.getPostalCode()));
	...
	public CompoundCommand getPropertiesEditionCommand(EditingDomain editingDomain) {
		...
		if (adress != null) {
			cc.append(SetCommand.create(editingDomain, adress, NonregPackage.eINSTANCE.getAdress_PostalCode(), new Integer(basePart.getText())));

			cc.append(SetCommand.create(editingDomain, adress, NonregPackage.eINSTANCE.getAdress_PostalCode(), new Integer(basePart.getTextRO())));
	...
	public EObject getPropertiesEditionObject(EObject source) {
		if (source instanceof Adress) {
			...
			adressToUpdate.setPostalCode(new Integer(basePart.getText()).intValue());

			adressToUpdate.setPostalCode(new Integer(basePart.getTextRO()).intValue());
	...
	public void firePropertiesChanged(PropertiesEditionEvent event) {
		...
		if (PropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(editing_mode)) {
			...
			if (diag != null && diag.getSeverity() != Diagnostic.OK) {
				if (NonregViewsRepository.Text.text == event.getAffectedEditor())
					basePart.setMessageForText(diag.getMessage(), IMessageProvider.ERROR);
				if (NonregViewsRepository.Text.textRO == event.getAffectedEditor())
					basePart.setMessageForTextRO(diag.getMessage(), IMessageProvider.ERROR);


			} else {
				if (NonregViewsRepository.Text.text == event.getAffectedEditor())
					basePart.unsetMessageForText();
				if (NonregViewsRepository.Text.textRO == event.getAffectedEditor())
					basePart.unsetMessageForTextRO();
