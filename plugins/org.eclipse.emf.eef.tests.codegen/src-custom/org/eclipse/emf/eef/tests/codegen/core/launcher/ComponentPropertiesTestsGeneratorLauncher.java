		...
		for (GenEditionContext genEditionContext : eefGenModel.getEditionContexts()) {
			if (genEditionContext.isGenerateJunitTestCases()) {
				try {
					...
					final URI template = getTemplateURI("org.eclipse.emf.eef.tests.codegen", new Path("/org/eclipse/emf/eef/tests/codegen/main/cases/ComponentPropertiesTests.emtl"));
