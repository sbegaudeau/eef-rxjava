...
public class OpenWithEEFReflectiveEditor implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		ElementTreeSelectionDialog workspaceJarSelectionDialog =
			new ElementTreeSelectionDialog(window.getShell(), new WorkbenchLabelProvider(), new WorkbenchContentProvider());
		...
		if (firstResult instanceof IFile)
			try {
				PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(new FileEditorInput((IFile) firstResult), "org.eclipse.emf.eef.reflect.editor");
