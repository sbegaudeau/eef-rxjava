...
public class SimpleModelNavigationPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public SimpleModelNavigationPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject simpleModelNavigation, String editing_mode) {
		...
		partKey = NavigationViewsRepository.SimpleModelNavigation.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(NavigationViewsRepository.SimpleModelNavigation.Properties.index)) {
				basePart.setIndex(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, simpleModelNavigation.getIndex()));
			}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == NavigationViewsRepository.SimpleModelNavigation.Properties.index) {
			return NavigationPackage.eINSTANCE.getSimpleModelNavigation_Index();
		}
		if (editorKey == NavigationViewsRepository.SimpleModelNavigation.Properties.feature) {
			return NavigationPackage.eINSTANCE.getSimpleModelNavigation_Feature();
		}
		if (editorKey == NavigationViewsRepository.SimpleModelNavigation.Properties.discriminatorType) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (NavigationViewsRepository.SimpleModelNavigation.Properties.index == event.getAffectedEditor()) {
			simpleModelNavigation.setIndex((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (NavigationViewsRepository.SimpleModelNavigation.Properties.feature == event.getAffectedEditor()) {
		...
		if (NavigationViewsRepository.SimpleModelNavigation.Properties.discriminatorType == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (NavigationPackage.eINSTANCE.getSimpleModelNavigation_Feature().equals(msg.getFeature()) && basePart != null && isAccessible(NavigationViewsRepository.SimpleModelNavigation.Properties.feature))
				basePart.setFeature((EObject)msg.getNewValue());
			if (NavigationPackage.eINSTANCE.getSimpleModelNavigation_DiscriminatorType().equals(msg.getFeature()) && basePart != null && isAccessible(NavigationViewsRepository.SimpleModelNavigation.Properties.discriminatorType))
	...
	public boolean isRequired(Object key, int kind) {
		return key == NavigationViewsRepository.SimpleModelNavigation.Properties.feature;
	}
