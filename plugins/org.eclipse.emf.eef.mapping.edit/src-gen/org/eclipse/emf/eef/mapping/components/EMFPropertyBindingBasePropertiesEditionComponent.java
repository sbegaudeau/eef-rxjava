 ...
 * 
 *
 * $Id: EMFPropertyBindingBasePropertiesEditionComponent.java,v 1.1 2009/04/30 17:14:42 glefur Exp $
 */
...
public class EMFPropertyBindingBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public EMFPropertyBindingBasePropertiesEditionComponent(EObject eMFPropertyBinding, String mode) {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(String kind, String key) {
		if (eMFPropertyBinding != null && BASE_PART.equals(key)) {
				if (basePart == null) {
					...
					if (provider != null) {
						basePart = (EMFPropertyBindingPropertiesEditionPart)provider.getPropertiesEditionPart(eMFPropertyBinding, this, key, kind);
	...
	public void firePropertiesChanged(PathedPropertiesEditionEvent event) {
		...
		if (PathedPropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(mode)) {
			...
			if (MappingViewsRepository.EMFPropertyBinding.name == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, eMFPropertyBinding, MappingPackage.eINSTANCE.getAbstractPropertyBinding_Name(), event.getNewValue());
			if (MappingViewsRepository.EMFPropertyBinding.views == event.getAffectedEditor()) {
				if (PathedPropertiesEditionEvent.ADD == event.getKind())
					command = AddCommand.create(liveEditingDomain, eMFPropertyBinding, MappingPackage.eINSTANCE.getAbstractPropertyBinding_Views(), event.getNewValue());
				if (PathedPropertiesEditionEvent.REMOVE == event.getKind())
					command = RemoveCommand.create(liveEditingDomain, eMFPropertyBinding, MappingPackage.eINSTANCE.getAbstractPropertyBinding_Views(), event.getNewValue());
				if (PathedPropertiesEditionEvent.MOVE == event.getKind())
					command = MoveCommand.create(liveEditingDomain, eMFPropertyBinding, MappingPackage.eINSTANCE.getAbstractPropertyBinding_Views(), event.getNewValue(), event.getNewIndex());
			}
			if (MappingViewsRepository.EMFPropertyBinding.model == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, eMFPropertyBinding, MappingPackage.eINSTANCE.getEMFPropertyBinding_Model(), event.getNewValue());
