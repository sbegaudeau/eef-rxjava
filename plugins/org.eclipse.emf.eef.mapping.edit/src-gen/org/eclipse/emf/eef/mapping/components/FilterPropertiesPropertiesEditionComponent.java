...
public class FilterPropertiesPropertiesEditionComponent extends StandardPropertiesEditionComponent {
		...
		if (!isInitializing()) {
			...
			if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {			
				...
				if (MappingViewsRepository.FilterProperties.mandatory == event.getAffectedEditor()) {
					updateMandatory((Boolean)event.getNewValue());
				}
			}
	...
	private void updateName(java.lang.String newValue) {
		bindingFilter.setName(newValue);	
	}

	private void updateMandatory(Boolean newValue) {
		bindingFilter.setMandatory(newValue);	
	}
