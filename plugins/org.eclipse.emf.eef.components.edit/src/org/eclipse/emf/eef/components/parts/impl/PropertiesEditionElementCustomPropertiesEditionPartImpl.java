			...
			public void process(IStructuredSelection selection) {
				for (Iterator iter = selection.iterator(); iter.hasNext();) {
					...
					if (!viewsEditUtil.getVirtualList().contains(elem)) {
						...
						propertiesEditionComponent.firePropertiesChanged(new PathedPropertiesEditionEvent(PropertiesEditionElementCustomPropertiesEditionPartImpl.this, ComponentsViewsRepository.PropertiesEditionElement.views, PathedPropertiesEditionEvent.COMMIT, PathedPropertiesEditionEvent.ADD, null, viewsEditUtil.foundCorrespondingEObject(elem)));
					}
				}
			}
