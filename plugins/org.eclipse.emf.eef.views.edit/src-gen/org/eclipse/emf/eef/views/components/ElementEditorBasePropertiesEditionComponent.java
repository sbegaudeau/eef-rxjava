 ...
 * 
 *
 * $Id: ElementEditorBasePropertiesEditionComponent.java,v 1.1 2009/04/30 17:16:51 glefur Exp $
 */
...
public class ElementEditorBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public ElementEditorBasePropertiesEditionComponent(EObject elementEditor, String mode) {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(String kind, String key) {
		if (elementEditor != null && BASE_PART.equals(key)) {
				if (basePart == null) {
					...
					if (provider != null) {
						basePart = (ElementEditorPropertiesEditionPart)provider.getPropertiesEditionPart(elementEditor, this, key, kind);
	...
	public void firePropertiesChanged(PathedPropertiesEditionEvent event) {
		...
		if (PathedPropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(mode)) {
			...
			if (ViewsViewsRepository.ElementEditor.representation == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, elementEditor, ViewsPackage.eINSTANCE.getViewElement_Representation(), event.getNewValue());
			if (ViewsViewsRepository.ElementEditor.name == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, elementEditor, ViewsPackage.eINSTANCE.getViewElement_Name(), event.getNewValue());
			if (ViewsViewsRepository.ElementEditor.readOnly == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, elementEditor, ViewsPackage.eINSTANCE.getElementEditor_ReadOnly(), event.getNewValue());
