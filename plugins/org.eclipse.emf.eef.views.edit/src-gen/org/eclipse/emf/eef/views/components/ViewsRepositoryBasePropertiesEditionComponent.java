...
// FIXME NO VALID CASE INTO template public filterUpdater(editionElement : PropertiesEditionElement, view : View, pec : PropertiesEditionComponent) in widgetControl.mtl module, with the values : RepositoryKind, ViewsRepository, ViewsRepository.
		...
		if (!isInitializing()) {
			...
			if (IPropertiesEditionComponent.BATCH_MODE.equals(editing_mode)) {			
				if (ViewsViewsRepository.ViewsRepository.repositoryKind == event.getAffectedEditor()) {
					// FIXME INVALID CASE you must override the template 'invokeEObjectUpdater' for the case : RepositoryKind, ViewsRepository, ViewsRepository.
				}
	...
	private void updateName(java.lang.String newValue) {
		viewsRepository.setName(newValue);	
	}
