 ...
 * 
 *
 * $Id: SpecificElementEditorBasePropertiesEditionComponent.java,v 1.1 2009/04/30 17:16:51 glefur Exp $
 */
...
public class SpecificElementEditorBasePropertiesEditionComponent extends StandardPropertiesEditionComponent {
	 ...
	 * Default constructor
	 */
	public SpecificElementEditorBasePropertiesEditionComponent(EObject specificElementEditor, String mode) {
	...
	public IPropertiesEditionPart getPropertiesEditionPart(String kind, String key) {
		if (specificElementEditor != null && BASE_PART.equals(key)) {
				if (basePart == null) {
					IPropertiesEditionProvider provider = PropertiesEditionComponentService.getInstance().getProvider(specificElementEditor);
					if (provider != null) {
						basePart = (SpecificElementEditorPropertiesEditionPart)provider.getPropertiesEditionPart(specificElementEditor, this, key, kind);
	...
	public void firePropertiesChanged(PathedPropertiesEditionEvent event) {
		...
		if (PathedPropertiesEditionEvent.COMMIT == event.getState() && IPropertiesEditionComponent.LIVE_MODE.equals(mode)) {
			...
			if (ViewsViewsRepository.SpecificElementEditor.representation == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, specificElementEditor, ViewsPackage.eINSTANCE.getViewElement_Representation(), event.getNewValue());
			if (ViewsViewsRepository.SpecificElementEditor.name == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, specificElementEditor, ViewsPackage.eINSTANCE.getViewElement_Name(), event.getNewValue());
			if (ViewsViewsRepository.SpecificElementEditor.readOnly == event.getAffectedEditor())
				command = SetCommand.create(liveEditingDomain, specificElementEditor, ViewsPackage.eINSTANCE.getElementEditor_ReadOnly(), event.getNewValue());
