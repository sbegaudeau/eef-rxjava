...
public class WidgetPropertiesEditionPartImpl extends CompositePropertiesEditionPart implements ISWTPropertiesEditionPart, WidgetPropertiesEditionPart {
	...
	public void createControls(Composite view) { 
		CompositionSequence widgetStep = new BindingCompositionSequence(propertiesEditionComponent);
		widgetStep
	...
	protected Composite createNameText(Composite parent) {
		...
		name.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				if (propertiesEditionComponent != null)
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(WidgetPropertiesEditionPartImpl.this, ToolkitsViewsRepository.Widget.Properties.name, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.SET, null, name.getText()));
			}
		...
		SWTUtils.createHelpButton(parent, propertiesEditionComponent.getHelpContent(ToolkitsViewsRepository.Widget.Properties.name, ToolkitsViewsRepository.SWT_KIND), null); //$NON-NLS-1$
...
// End of user code
	...
	public void setName(String newValue) {
		...
		boolean eefElementEditorReadOnlyState = isReadOnly(ToolkitsViewsRepository.Widget.Properties.name);
		if (eefElementEditorReadOnlyState && name.isEnabled()) {
			name.setEnabled(false);
			name.setToolTipText(ToolkitsMessages.Widget_ReadOnly);
