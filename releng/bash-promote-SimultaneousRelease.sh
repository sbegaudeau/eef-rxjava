...
cd $BUILD_LOC
wget --no-check-certificate "https://build.eclipse.org/hudson/job/cbi-emf-eef-0.7-integration/lastSuccessfulBuild/artifact/build/*zip*/build.zip" -o $logFile
if [ ! -f build.zip ]; then echo "ERROR:build.zip (from Hudson) not found" >> $logFile; exit -2; fi
unzip -o build.zip >> $logFile
echo "[`date +%Y/%m/%d\ %H:%M`]: publishing milestone build ..." >> $logFile
mkdir -p $PROMO_LOGS_DIR
$ANT -f $RELENG_DIR/promote.xml -Dpromote.properties=$RELENG_DIR/promote-SimultaneousRelease.properties 2>/dev/null 1> $PROMO_LOGS_DIR/cbi-emf-eef-promo-SimultaneousRelease.txt
